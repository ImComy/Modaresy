  ...
  return (
    ...
    >
      <div className="bg-gradient-to-br from-muted/10 to-muted/30 border border-border/50 rounded-xl shadow-lg p-6">
        ...
        <p className="text-muted-foreground text-sm">{t('managePricingAndOffers')}</p>
        <div className="mt-6 text-center text-muted-foreground">
          <DollarSign className="mx-auto mb-4 h-16 w-16 opacity-30 text-primary" />
          <p className="text-lg font-semibold mb-1">{t('comingSoon')}</p>
          <p className="text-sm">{t('pricesAndOffersDesc')}</p>
        </div>
      </div>
    </motion.div>
  );
