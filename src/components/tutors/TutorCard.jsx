      ...
      return (
        ...
        >
          <Link to={`/tutor/${tutor.id}`} className="block h-full">
           ...
           <img src={tutor.bannerimg || 'https://placehold.co/600x400'} alt={tutor.name} className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-105" /> {/* Slightly faster scale */}
          ...
          <div className="flex items-start gap-3 mb-2">
             <Avatar className="h-12 w-12 border-2 border-primary flex-shrink-0 rounded-sm">
               <AvatarImage src={tutor.img} alt={tutor.name} radius="rounded-sm"/>
               <AvatarFallback radius="rounded-sm">{tutor.name?.split(' ').map(n => n[0]).join('')}</AvatarFallback>
             </Avatar>
